...
output:
  flexdashboard::flex_dashboard:
    ...
    navbar:
    - href: https://github.com/BojanMakivic/An-interactive-ShinyApp-to-track-COVID-19-in-real-time
...
vacc <- read.csv("https://raw.githubusercontent.com/BojanMakivic/covid-19-data/master/public/data/vaccinations/vaccinations.csv")
...
ZoomButton <- data.frame(
  Unit = c("DD", "DD", "MAX"), multiple = c(10, 15, 1),
...
ggplotRegression <- function(fit) {
  ...
  ggplot(fit$model, aes_string(
    y = names(fit$model)[1],
    x = names(fit$model)[2],
    x1 = names(fit$model)[3],
    label = names(fit$model)[4]
  )) +
    geom_point(na.rm = TRUE) +
    labs(x = paste(names(fit$model)[2]), x1 = paste(names(fit$model)[3]), y = paste(names(fit$model)[1])) +
